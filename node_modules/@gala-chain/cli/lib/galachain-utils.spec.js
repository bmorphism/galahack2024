...
describe("getDeploymentResponse", () => {
    ...
    it("should get deployment response", async () => {
        ...
        // When
        const response = await (0, src_1.getDeploymentResponse)({ privateKey, isTestnet });
        // Then
    ...
    it("should fail when get deployment response", async () => {
        ...
        // When
        expect(async () => await (0, src_1.getDeploymentResponse)({ privateKey, isTestnet })).rejects.toThrowError(`Service Portal respond with status 401`);
    });
});
describe("deployChaincode", () => {
    ...
    it("should get deployChaincode response", async () => {
        ...
        execSync.mockImplementation(() => '[{"contractName":"AppleContract"},{"contractName":"GalaChainToken"},{"contractName":"PublicKeyContract"}]');
        // When
        const response = await (0, src_1.deployChaincode)({ privateKey, isTestnet, imageTag });
        // Then
        expect(response.status).toEqual("CH_CREATED");
    });
    it("should read a file with a private key and get deployChaincode response", async () => {
        ...
        const postDeployChaincodePrivateKey = {
            privateKey: await (0, src_1.getPrivateKey)(undefined),
            isTestnet,
            imageTag
        };
        // When
        const response = await (0, src_1.deployChaincode)(postDeployChaincodePrivateKey);
        // Then
        expect(response.status).toEqual("CH_CREATED");
    });
    it("should ask for private key and get deployChaincode response", async () => {
        ...
        jest.spyOn(core_1.ux, "prompt").mockResolvedValueOnce(privateKey);
        const postDeployChaincodePrivateKey = {
            privateKey: await (0, src_1.getPrivateKey)(undefined),
            isTestnet,
            imageTag
    ...
    it("should fail when post deployChaincode", async () => {
        ...
        execSync.mockImplementation(() => '[{"contractName":"AppleContract"},{"contractName":"GalaChainToken"},{"contractName":"PublicKeyContract"}]');
        // When
        expect(async () => await (0, src_1.deployChaincode)({ privateKey, isTestnet, imageTag })).rejects.toThrowError(`Service Portal respond with status 401`);
    });
});

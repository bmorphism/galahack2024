...
const connect_1 = tslib_1.__importDefault(require("../../../src/commands/connect"));
...
describe("Connect Command", () => {
    it("should check if it connects to a new chaincode", async () => {
        ...
        // When
        await connect_1.default.run([fakePrivateKey]);
        // Then
        expect(result.join()).toContain("You are now connected! Chaincode chaincode-name and Channel channel-name.");
    });
    it("should not find private key", async () => {
        ...
        jest.spyOn(core_1.ux, "prompt").mockResolvedValueOnce(fakePrivateKey);
        // When
        await connect_1.default.run([]);
        // Then
    ...
    it("should fail when invalid private key", async () => {
        ...
        await expect(connect_1.default.run([fakeInvalidPrivateKey])).rejects.toThrow();
    });
});
